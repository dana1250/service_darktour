...
<html>
	<head>
		...
		<script>
	    function idDelete(delCOURSE_CODE){
	       location.href = "deleteCourse.jsp?COURSE_CODE=" + delCOURSE_CODE;   //get방식으로 삭제할아이디를 넘김     
	    }
		</script>
	</head>
	<body>	
	...
	<div class="gtco-section">
		<div class="gtco-container">
			<div class="row">
				<div class="col-md-8 col-md-offset-2 text-center gtco-heading">				
					<jsp:useBean id="dao" class="darktour.JDBC_coursesDAO" />
				    <%
				    ArrayList<CourseVO> list = dao.getCourselist();
				    %>
					<h2>Course List</h2>
    				<form action="deleteCourse.jsp" method="post" id="multidelete">
				</div>
			</div>
			<div class="row">
		...
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				<thead>
					<tr>
						<th style="background-color: #eeeeee; text-align: center;">코스 코드</th>
						<th style="background-color: #eeeeee; text-align: center;">여행 코스</th>
				...
				<tbody>
				    ...
				    %> 
			        <tr>
			            <td><%=vo.getCOURSE_CODE() %></td>
			            <td><%=vo.getCONTENTS() %></td>
			            <td><input type="button" value="삭제" onclick="idDelete('<%=vo.getCOURSE_CODE() %>');"></td>
			        </tr>				               
				    <%
				    }
				     %>
