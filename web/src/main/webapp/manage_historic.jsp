<%@page import="darktour.HistoricVO"%>
...
<html>
	<head>
		...
		<script>
	    function idDelete(delHISTORIC_NUM){
	       location.href = "deleteHistoric.jsp?HISTORIC_NUM=" + delHISTORIC_NUM;   //get방식으로 삭제할아이디를 넘김     
	    }
		</script>
	</head>
	<body>	
							...
							<div class="col-md-12 col-md-offset-0 text-left">	
								<div class="row row-mt-15em">
									<div class="col-md-7 mt-text animate-box" data-animate-effect="fadeInUp">
										<h1>Historic Site Management</h1>	
									</div>
								</div>
							</div>
						</div>
	...
	<div class="gtco-section">
		<div class="gtco-container">
			<div class="row">
				<div class="col-md-8 col-md-offset-2 text-center gtco-heading">				
				    ...
				    <%
				    ArrayList<HistoricVO> list = dao.getHistoriclist();
				    %>
					<h2>Historic Site List</h2>
		...
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				<thead>
					<tr>
						<th style="background-color: #eeeeee; text-align: center;">유적지 번호</th>
				...
				<tbody>
					 <%
				    for(HistoricVO vo : list){
				    %> 
			        <tr>
			            <td><%=vo.getHISTORIC_NUM() %></td>
			            <td><%=vo.getNAME() %></td>
			            <td><%=vo.getEXPLAIN_HIS() %></td>
			            <td><%=vo.getADDRESS() %></td>
			            <td><input type="button" value="삭제" onclick="idDelete('<%=vo.getHISTORIC_NUM() %>');"></td>
			        </tr>				               
				    <%
				    }
				     %>
